...
#define inmell(x) {x=getc(mcode);lest=1;}

#if SIZEOF_LONG == 8
#define inmellival(x) {x=getc(mcode);x|=((long)getc(mcode))<<8;\
x|=((long)getc(mcode))<<16;x|=((long)getc(mcode))<<24;\
x|=((long)getc(mcode))<<32;x|=((long)getc(mcode))<<40;\
x|=((long)getc(mcode))<<48;x|=((long)getc(mcode))<<56;}
#else
#define inmellival(x) {x=getc(mcode);x|=((long)getc(mcode))<<8;\
x|=((long)getc(mcode))<<16;x|=((long)getc(mcode))<<24;}
#endif
...
static struct EXP *expdisplay[MAXEXPDISPLAY];
...
#if 0
static struct EXP *classlist;
static struct EXP *cb;			/* Liste av klassene i en prefikskjede,med 
				 * med laveste prefiksnivå først. Brukes
				 * for å få riktig rekkefølge p}
				 * parameterene ved generering av et nytt
				 * klasse objekt. */
#endif
static char newarg;			/* Hvis newarg så skal nextclassparam kalles
...
char not_reached;
static char computeconst ();
static char setdanger_const ();
     ...
     struct EXP *re;
{
  ...
  switch (TOKEN)
	...
	s = (char *) xmalloc (strlen (l) + strlen (r) + 1);
	(void) strcpy (s, l);
	(void) strcat (s, r);
	TOKEN = MTEXTKONST;
	VALUE.tval.txt = s;
